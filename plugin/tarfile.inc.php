...
define('FILENAME_ENCORDING','SJIS');

// 最大アップロードサイズ
define('TAR_MAX_FILE_SIZE',1024);	// Kbyte単位

/////////////////////////////////////////////////
// ファイル名の拡張子
define('TAR_FILENAME_TAR' , '.tar' );
define('TAR_FILENAME_GZIP', '.tar.gz' );
...
// action定義
define('TAR_ACTION_DOWNLOAD','act_download');	// ダウンロード
define('TAR_ACTION_UPLOAD'  ,'act_upload');		// アップロード
...
//	if (pkwk_login($pass))	// for pukiwiki-1.4.4
	if ($pass !== NULL)
	{
		...
		{
			if($act == TAR_ACTION_DOWNLOAD)
			{
				// ファイルの作成・ダウンロード
				$body = plugin_tarfile_download();
			}
			else if($act == TAR_ACTION_UPLOAD)
			{
				// ファイルのアップロード
...
function plugin_tarfile_download()
{
	...
	if($tar->create(CACHE_DIR,$arc_kind))
	{
		...
		if($bk_wiki)
		{
			$filecount .= $tar->add(DATA_DIR,'^[0-9A-F]+\.txt',$namedecode);
		}
		if($bk_attach)
		{
			$filecount .= $tar->add(UPLOAD_DIR,'^[0-9A-F_]+',$namedecode);
		}
		if($bk_backup)
		{
			$filecount .= $tar->add(BACKUP_DIR,'^[0-9A-F]+\.gz',$namedecode);
		}
		$tar->close();
...
function download_tarfile($name,$arc_kind)
{
	...
	{
		// 〜.tar.gz
		$filename .= TAR_FILENAME_GZIP;
	}
	else
	{
		// 〜.tar
		$filename .= TAR_FILENAME_TAR;
	}
...
function plugin_tarfile_disp_form()
{
	...
	$act_down  = TAR_ACTION_DOWNLOAD;
	$act_up    = TAR_ACTION_UPLOAD;
	$maxsize   = TAR_MAX_FILE_SIZE * 1024;
	$maxsizekb = TAR_MAX_FILE_SIZE;
...
<br />
  <input type="checkbox" name="namedecode" /> ファイル名をページ名に変換する<br />
</p>
...
<p><strong>[重要] 同じ名前のファイルは上書きされますので、十分ご注意ください。</strong></p>
<p><span class="small">
...
class compact_tarlib
{
	...
	function create( $odir, $kind = ARCFILE_GZIP )
	{
		...
		if($kind == ARCFILE_GZIP)
		{
			$this->fp = gzopen( $tname, 'wb');	// gzip圧縮をする場合
			$this->arc_kind = ARCFILE_GZIP;
		}
		else
		{
...
//			if ($type == 0 && preg_match("/$pattern/",$name) )
	...
	function add( $dir, $mask, $decode = false )
	{
		...
		foreach($files as $name)
		{
			...
			if( $decode==true )
			{
				...
				{
					// ファイル名の文字コードを変換
					$filename = mb_convert_encoding($filename, FILENAME_ENCORDING);
				}
			}
			else
			{
	...
	function write_data($header, $body, $size)
	{
		...
		flock($this->fp, LOCK_EX);
		// ヘッダ出力
		fwrite( $this->fp, $header, TAR_HDR_LEN );
		// データ出力
		fwrite( $this->fp, $body, $size);
		// サイズ調整(あまった部分を0でフィル)
		fwrite( $this->fp, $this->dummydata, $fixsize);
		flock($this->fp, LOCK_UN);
	}
}
