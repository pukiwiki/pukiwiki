...
//
// Copyright: 2003 By 閑舎 <raku@rakunet.org>
//
...
function plugin_amazon_get_asin_title() {
  ...
  if (file_exists(CACHE_DIR) === false or is_writable(CACHE_DIR) === false) $nocachable = 1; // キャッシュ不可の場合
...
//    }
  ...
  if ($nocache == 1 and $nocachable != 1) plugin_amazon_cache_title_save($title, CACHE_DIR); // タイトルがあればキャッシュに保存
  return $title;
}
...
function plugin_amazon_cache_image_fetch($dir) {
  ...
  if ($get_img) {
    ...
    if ($body == "" or $size[1] <= 1) { // 通常は1が返るが念のため0の場合も(reimy)
