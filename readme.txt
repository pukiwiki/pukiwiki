...
設置方法
    ...
    5.  attach.inc.php内で指定したディレクトリを作成します。
        (デフォルトは attach)

    6.  counter.inc.php内で指定したディレクトリを作成します。
        (デフォルトは counter)

    7.  trackback.php内で指定したディレクトリを作成します。
        (デフォルトは trackback)

    8.  サーバ上のファイルおよびディレクトリのパーミッションを確認します。
        ファイルのパーミッションについては次項を参照してください。

    9.  pukiwiki.phpにブラウザからアクセスします。
